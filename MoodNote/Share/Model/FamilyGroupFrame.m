...
-(void)setFamilyGroup:(FamilyGroup *)familyGroup
{
    ...
    if([self.familyGroup.replys count])
    {
        ...
        for (int i = 0; i < [self.familyGroup.replys count]; i++) {
            ...
            YFCommentModel *model = [self.familyGroup.replys objectAtIndex:i];
            
            CGSize replyLabelSize = [self sizeWithString:[NSString stringWithFormat:@"%@:%@",model.commentUserName,model.commentText] font:replytextFont maxSize:CGSizeMake(screenWidth - 2*padding - nameLabelX, MAXFLOAT)];
