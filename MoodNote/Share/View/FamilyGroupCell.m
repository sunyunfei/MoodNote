...
-(void)settingtData
{
    ...
    //创建头像
    [self.iconView sd_setImageWithURL:[NSURL URLWithString:familyGroup.icon]];
    ...
    for (int i = 0; i < [familyGroup.pictures count]; i++) {
        ...
        [self.contentView addSubview:pictureView];
        [self.picturesView addObject:pictureView];
        [((UIImageView *)[self.picturesView objectAtIndex:i]) sd_setImageWithURL:[NSURL URLWithString:[familyGroup.pictures objectAtIndex:i]]];
    }
    ...
    for (int i = 0; i < [familyGroup.replys count]; i++) {
        ...
        YFCommentModel *model = [familyGroup.replys objectAtIndex:i];
        if (model.commentUserName != nil || model.commentUserName.length > 0) {
            
            NSMutableAttributedString *str = [[NSMutableAttributedString alloc]initWithString:model.commentUserName];
            [str addAttribute:NSForegroundColorAttributeName value:[UIColor redColor] range:NSMakeRange(0, str.length)];
            // NSAttributedString *str2 =[[NSAttributedString alloc]initWithString:model.commentText];
            [str appendAttributedString:[[NSAttributedString alloc]initWithString:[NSString stringWithFormat:@":%@",model.commentText]]];
            replyLabel.attributedText = str;
            
        }else{
        
            replyLabel.text = model.commentText;
        }
        
        //NSString *searchText = [NSString stringWithFormat:@"%@:%@",model.commentUserName,model.commentText];
...
- (void)clickLabel:(UITapGestureRecognizer *)sender{
    ...
    YFCommentModel *model = [self.familyGroupFrame.familyGroup.replys objectAtIndex:index];
    if (self.labelBlock) {
        
        self.labelBlock(model.commentUserName,self.replyButton.tag);
    }
}
